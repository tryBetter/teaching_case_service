...
本系统实现了基于角色的访问控制（RBAC），支持五种用户角色：管理员、教师组长、教师、助教、学生。不同角色具有不同的权限，可以访问和操作不同的模块。
...
### 角色枚举
```sql
CREATE TYPE "UserRole" AS ENUM ('ADMIN', 'TEACHER_LEADER', 'TEACHER', 'ASSISTANT', 'STUDENT');
```
